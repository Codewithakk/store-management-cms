                ...
                <div>
                  ...
                  {isStatusFinal(selectedOrder.status) ? (
                    <div className="p-3 bg-gray-50 border rounded text-gray-600 text-sm">
                      This order has reached the final status <span className="font-medium">{selectedOrder.status}</span>. 
                      No further status updates are allowed.
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      {Object.keys(statusColors).map((status) => {
                        const selectableStatuses = getSelectableStatuses(selectedOrder.status);
                        return (
                          <Button
                            key={status}
                            variant={
