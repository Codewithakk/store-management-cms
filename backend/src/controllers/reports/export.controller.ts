...
export const generateStaffActivityReport = async ({ workspaceId, filters }: ReportParams) => {
    ...
    return staffMembers.map((staff) => ({
        ...
        processingCount: staff.OrderStatusHistory.filter((h) => h.status === 'PROCESSING').length,
        completedCount: staff.OrderStatusHistory.filter((h) => h.status === 'DELIVERED').length,
