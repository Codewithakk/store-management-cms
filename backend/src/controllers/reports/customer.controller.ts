...
export const getCustomerActivityReport = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const customersWithStats = activeCustomers.map((customer) => ({
            ...
            lastOrderDate: customer.orders.length > 0 ? new Date(Math.max(...customer.orders.map((o) => o.placedAt.getTime()))) : null
        }))
...
export const generateCustomerLoyaltyReport = async ({ workspaceId, filters }: ReportParams) => {
    ...
    return customers.map((customer) => {
        ...
        const firstOrder = customer.orders.reduce((min, o) => (o.placedAt < min ? o.placedAt : min), new Date())
        const lastOrder = customer.orders.reduce((max, o) => (o.placedAt > max ? o.placedAt : max), new Date(0))
        ...
        return {
            ...
            daysSinceLastOrder: Math.floor((new Date().getTime() - lastOrder.getTime()) / (1000 * 60 * 60 * 24))
        }
    })
}
...
export const generateCustomSalesReport = async ({ workspaceId, filters }: ReportParams) => {
    const whereClause = {
        ...
        ...(filters.paymentStatus && {
            paymentStatus: { in: Array.isArray(filters.paymentStatus) ? filters.paymentStatus : [filters.paymentStatus] }
        })
    }
