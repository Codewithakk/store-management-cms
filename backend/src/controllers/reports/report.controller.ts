...
import { DateRangeFilter, PaginationFilter, ReportType, SortFilter } from '../../types/types'
import { reportService } from '../../services/report.service'
import { reportQuerySchema } from '../../validation/report.validation'
...
export const generateReportData = async (type: ReportType, params: ReportParams) => {
    switch (type) {
        case 'sales':
...
export const buildOrderWhereClause = (params: ReportParams): Prisma.OrderWhereInput => ({
    ...
    status: params.filters.status ? { in: Array.isArray(params.filters.status) ? params.filters.status : [params.filters.status] } : undefined,
...
export const generateInventoryReport = async ({ workspaceId, filters }: ReportParams) => {
    ...
    const sortedData = applySorting(inventoryData, filters.sortBy, filters.sortOrder as 'asc' | 'desc')
...
export const generateCustomerReport = async ({ workspaceId, filters }: ReportParams) => {
    ...
    const customers = await prisma.user.findMany({
        ...
        include: {
            ...
            orders: {
                ...
                take: filters.lastOrders ? parseInt(filters.lastOrders) : undefined
            }
        }
    })
    ...
    const customerData = customers.map((customer) => ({
        ...
        avgOrderValue: customer._count.orders ? customer.orders.reduce((sum, o) => sum + o.totalAmount, 0) / customer._count.orders : 0,
        lastOrderDate: customer.orders[0]?.placedAt || null
    }))

    const sortedData = applySorting(customerData, filters.sortBy, filters.sortOrder as 'asc' | 'desc')
...
export const generateEmployeePerformanceReport = async ({ workspaceId, filters }: ReportParams) => {
    ...
    return users.map((user) => ({
        ...
        processedOrders: user.OrderStatusHistory.filter((h) => h.status === 'PROCESSING').length,
        completedOrders: user.OrderStatusHistory.filter((h) => h.status === 'DELIVERED').length,
...
export const generateFinancialSummaryReport = async ({ workspaceId, filters }: ReportParams) => {
    ...
    const [totalRevenue, totalOrders, paymentStatusCounts, paymentMethodStats, refunds] = await Promise.all([
...
export const generateProductPerformanceReport = async ({ workspaceId, filters }: ReportParams) => {
    ...
    const sortedData = applySorting(productStats, filters.sortBy, filters.sortOrder as 'asc' | 'desc')
