...
const getPermissionsCacheKey = (workspaceId: number, role: string) => `workspace:${workspaceId}:role:${role}:permissions`;
const getWorkspaceCacheKey = (userId: string) => `workspaces:${userId}`;
const getWorkspaceUsersCacheKey = (workspaceId: number) => `workspace:${workspaceId}:users`;
const getWorkspaceDataCacheKey = (workspaceId: number) => `workspace:${workspaceId}:data`;
const getAdminWorkspacesCacheKey = (userId: string) => `admin:workspaces:${userId}`;
const invalidateWorkspaceCache = async () => {
  const keys = await redisClient.keys('workspaces:search:*');
  if (keys.length) {
...
export const getWorkspacesByUserId = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    let workspaces = await getFromCache(cacheKey);
    ...
    if (!workspaces) {
      ...
      await setToCache(cacheKey, workspaces, RedisTTL.ACCESS_TOKEN);
      return httpResponse(req, res, 200, 'Workspaces fetched from DB and cached', workspaces);
    }

    return httpResponse(req, res, 200, 'Workspaces fetched from cache', workspaces);
...
export const updateWorkspace = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const normalizeField = (val: any) => (val === 'null' || val === undefined || val === '' ? null : val)
    ...
    // Invalidate related caches
    await Promise.all([
      deleteFromCache(getWorkspaceDataCacheKey(workspaceId)),
      deleteFromCache(getWorkspaceCacheKey(userId)),
      deleteFromCache(getAdminWorkspacesCacheKey(userId)),
      invalidateWorkspaceCache(),
    ]);
...
export const removeUserFromWorkspace = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Invalidate related caches
    await Promise.all([
      deleteFromCache(getWorkspaceUsersCacheKey(workspaceId)),
      deleteFromCache(getWorkspaceDataCacheKey(workspaceId)),
      invalidateWorkspaceCache()
    ]);
...
export const allDeleteWorkspace = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Invalidate caches for all deleted workspaces
    const cacheDeletions = workspaceIds.map(id =>
      Promise.all([
        deleteFromCache(getWorkspaceDataCacheKey(id)),
        deleteFromCache(getWorkspaceUsersCacheKey(id)),
        ...Object.values(Role).map(role => deleteFromCache(getPermissionsCacheKey(id, role)))
      ])
    );
    await Promise.all([
      ...cacheDeletions,
...
export const deleteWorkspaceById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    await Promise.all([
      ...
      deleteFromCache(getAdminWorkspacesCacheKey(userId)),
      invalidateWorkspaceCache(),
      ...Object.values(Role).map(role => deleteFromCache(getPermissionsCacheKey(workspaceId, role)))
    ]);
...
export const getAdminWorkspaces = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const cacheKey = getAdminWorkspacesCacheKey(userId);
    let workspaces = await getFromCache(cacheKey);

    if (!workspaces) {
      workspaces = await workspaceService.getAdminWorkspaces(userId);
      await setToCache(cacheKey, workspaces, RedisTTL.ACCESS_TOKEN);
    }
...
export const inviteUserToWorkspace = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const invitation = await workspaceService.inviteUserToWorkspace({ email, role: normalizedRole, workspaceId }, userId)
    return invitation
      ? httpResponse(req, res, 201, 'Invitation sent', { invitation })
      : httpResponse(req, res, 200, 'User is already part of the workspace')
  } catch (error) {
...
export const acceptInvite = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  //   // Check if result contains redirectUrl
  //   if (result.redirectUrl) {
  //     return httpResponse(req, res, 200, 'Invitation accepted successfully', { redirectUrl: result.redirectUrl })
  //   } else {
  //     return httpResponse(req, res, 200, 'Invitation accepted successfully')
  //   }
  // } catch (error) {
  ...
  try {
    ...
    // Perform a real browser redirect if redirectUrl is available
    if (result.redirectUrl) {
      return res.redirect(result.redirectUrl); // ✅ Redirect user to login
    } else {
      return httpResponse(req, res, 200, 'Invitation accepted successfully');
    }
  } catch (error) {
...
export const setRolePermission = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result = await workspaceService.setRolesPermissionService(Number(workspaceId), normalizedRole, permission)
    // 🔄 Invalidate cache
    await deleteFromCache(getPermissionsCacheKey(workspaceId, normalizedRole));
...
export const assignRolePermission = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    await workspaceService.setRolesPermissionService(workspaceId, upperRole, permissions)
    await deleteFromCache(getPermissionsCacheKey(workspaceId, upperRole));
...
export const getWorkspaceUsers = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const cacheKey = getWorkspaceUsersCacheKey(workspaceId);
    let users = await getFromCache(cacheKey);

    if (!users) {
      users = await workspaceService.getWorkspaceUsers(workspaceId, userId);
      await setToCache(cacheKey, users, RedisTTL.ACCESS_TOKEN);
    }
...
export const changeInvitationStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const invitationId = parseInt(req.params.invitationId)
    ...
    await workspaceService.changeInvitationStatus(invitationId, status as InvitationStatus);
...
export const getRolePermissions = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const cacheKey = getPermissionsCacheKey(workspaceId, role);
    const cached = await getFromCache(cacheKey);
    if (cached) {
      return httpResponse(req, res, 200, 'Permissions fetched from cache', cached);
    }

    const permissions = await workspaceService.getRolePermissions(workspaceId, role);
    await setToCache(cacheKey, permissions, RedisTTL.ACCESS_TOKEN);
...
export const removeRolePermission = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    await workspaceService.removeRolePermission(workspaceId, role, permission, userId)
    // 🔄 Invalidate cache
    await deleteFromCache(getPermissionsCacheKey(workspaceId, role));
    return httpResponse(req, res, 200, 'Permission removed successfully')
  } catch (error) {
    return httpError(next, error, req)
  }
...
export const exportWorkspaceData = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const invitationPage = parseInt(req.query.invitationPage as string) || 1
    const invitationPageSize = parseInt(req.query.invitationPageSize as string) || 10
    const cacheKey = getWorkspaceDataCacheKey(workspaceId) + `:export:u${userPage}:${userPageSize}:i${invitationPage}:${invitationPageSize}`;
    let data = await getFromCache(cacheKey);

    if (!data) {
      data = await workspaceService.exportWorkspaceData(workspaceId, userPage, userPageSize, invitationPage, invitationPageSize);
      await setToCache(cacheKey, data, RedisTTL.ACCESS_TOKEN);
    }
