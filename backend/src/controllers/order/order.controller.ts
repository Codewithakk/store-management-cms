...
export const getOrdersAddress = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const addresses = await orderService.getOrdersAddress(authUserId)
...
export const addOrderItem = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    const orderItem = await orderService.addOrderItem(workspaceId, orderId, { variantId, quantity, price })
...
export const assignDeliveryPartner = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!orderId || !userId) {
      console.log('Invalid input:', { orderId, userId });
      res.status(400).json({
        success: false,
    ...
    if (isNaN(workspaceId)) {
      console.log('Invalid workspaceId:', req.params.workspaceId);
      res.status(400).json({
        success: false,
    ...
    const result = await orderService.assignDeliveryPartner({ orderId, userId }, workspaceId);
...
export const getStaffByWorkspace = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const workspaceId = parseInt(req.params.workspaceId, 10);
  ...
  try {
    const result = await orderService.getOrdegetStaffByWorkspace(workspaceId, req.user?.userId as string);
...
export const getOrdersByWorkspace = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  const workspaceId = Number(req.params.workspaceId);
  ...
  try {
    const orders = await orderService.getOrdersByWorkspace(workspaceId, req.user?.userId);
    if (!orders) {
      return httpResponse(req, res, 404, 'Orders not found')
    }
    return httpResponse(req, res, 200, 'Orders retrieved successfully', orders);
  } catch (error) {
    return httpError(next, error, req);
