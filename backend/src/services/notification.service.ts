...
export const notificationService = {
    ...
    deleteNotification: async ({ notificationId, workspaceId, userId, userRoles }: DeleteNotificationParams) => {
        // Validate inputs
        ...
        const hasAdminOrManagerRole = userRoles.some(
            (roleObj) => [Role.ADMIN, Role.MANAGER, Role.STAFF, Role.CUSTOMER].includes(roleObj.role) && roleObj.workspaceId === workspaceId
        )
    ...
    async update(notificationId: string, data: Partial<z.infer<typeof createNotificationSchema>>) {
