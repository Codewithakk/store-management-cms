...
export const productService = {
    createProduct: async (workspaceId: number, categoryId: string, data: ProductInput) => {
        try {
            ...
            if (variants.length > 0) {
                const skusToCheck = variants.map((v) => v.sku).filter((sku): sku is string => typeof sku === 'string' && sku.trim() !== '')
                ...
                if (skusToCheck.length) {
                    ...
                    if (newVariants.length < variants.length) {
                        const skippedSKUs = variants.filter((v) => v.sku && existingSKUSet.has(v.sku)).map((v) => v.sku)
    ...
    updateProduct: async (workspaceId: number, productId: string, data: Partial<ProductInput>) => {
        try {
            ...
            // Use updateData to update the product
    ...
    getProductStats: async (workspaceId: number) => {
      ...
      `
        ...
        const { total: totalProducts, active: activeProducts, inactive: inactiveProducts } = productStats[0]
        const { total: totalVariants, available: availableVariants, out_of_stock: outOfStockVariants } = variantStats[0]
    ...
    bulkUploadProducts: async (workspaceId: number, products: (ProductInput & { categoryId: string })[]) => {
        const created = await Promise.all(products.map((p) => productService.createProduct(workspaceId, p.categoryId, p)))
        return created
    },
