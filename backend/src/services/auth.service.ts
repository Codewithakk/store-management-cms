...
export const authService = {
    ...
    signInService: async (email: string, password: string) => {
        ...
        // Fetch the user's roles including workspaceId
        const userRoles = await prisma.userRole.findMany({
        ...
        // Identify if there's only one STAFF or MANAGER role
        const staffOrManagerRoles = userRoles.filter(ur =>
            ur.role === 'STAFF' || ur.role === 'MANAGER'
        )

        let singleWorkspaceId: number | undefined = undefined
        if (staffOrManagerRoles.length === 1) {
            singleWorkspaceId = staffOrManagerRoles[0].workspaceId ?? undefined
        }
        ...
        return {
            ...
            user: {
                ...
                workspaceId: singleWorkspaceId // Include only if single role is STAFF or MANAGER
            }
        }
    },
    ...
    updateUserProfileService: async (
        userId: string,
        data: Partial<Pick<User, 'firstName' | 'lastName' | 'phone'>> & { location?: Partial<Location> },
        ...
        // Only allow specific user fields
        if (firstName && typeof firstName === 'string') updatePayload.firstName = firstName.trim();
        if (lastName && typeof lastName === 'string') updatePayload.lastName = lastName.trim();
        if (phone && typeof phone === 'string') updatePayload.phone = phone.trim();

        // Handle profile image (if Cloudinary or local path is provided)
        if (profileImageFile?.path) {
            updatePayload.profileImageUrl = profileImageFile.path;
        }
        ...
        // Optional: add or replace user's location
        if (location && typeof location === 'object') {
            const {
                name,
                address,
            ...
            const newLocation = await prisma.location.create({
                data: {
                    name: name?.trim() || `${firstName || 'User'}'s location`,
                    address: address?.trim() || '',
                    street: street?.trim() || '',
                    city: city?.trim() || '',
                    region: region?.trim() || '',
