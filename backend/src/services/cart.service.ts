...
export const addToCart = async (userId: string, variantId: string, quantity: number = 1) => {
...
export const updateCartItemByVariant = async (userId: string, variantId: string, quantity: number) => {
...
export const getCartTotal = async (userId: string) => {
    ...
    return cartItems.reduce((total, item) => total + item.quantity * item.variant.price, 0)
}
...
export const checkCartItemsAvailability = async (userId: string) => {
    ...
    const unavailableItems = cartItems.filter((item) => !item.variant.isAvailable || item.quantity > item.variant.stock)
