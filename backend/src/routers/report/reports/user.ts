...
const router = Router()
router.get('/:workspaceId/reports/staff-activity', authMiddleware, roleRestriction([Role.ADMIN]), (req, res, next) => {
...
router.get('/:workspaceId/reports/user-signups', authMiddleware, roleRestriction([Role.ADMIN, Role.MANAGER]), (req, res, next) => {
    const workspaceId = parseInt(req.params.workspaceId, 10)
    const filters = req.query
