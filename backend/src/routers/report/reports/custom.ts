...
router.get('/:workspaceId/reports/custom-sales', authMiddleware, roleRestriction([Role.ADMIN, Role.MANAGER]), (req, res, next) => {
...
router.get('/:workspaceId/reports/custom-product', authMiddleware, roleRestriction([Role.ADMIN, Role.MANAGER]), (req, res, next) => {
    const workspaceId = parseInt(req.params.workspaceId, 10)
    const filters = req.query
