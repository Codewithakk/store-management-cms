...
const roleRestriction = (allowedRoles: Role[]) => {
    return (req: AuthRequest, res: Response, next: NextFunction): void => {
        ...
        const currentWorkspaceId = req.params.workspaceId ? parseInt(req.params.workspaceId) : user.workspaceId // fallback if not in params
        ...
        const hasAllowedRole = user.roles.some((role: UserRole) => allowedRoles.includes(role.role) && role.workspaceId === currentWorkspaceId)
