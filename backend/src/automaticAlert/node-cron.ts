...
export async function getLowStockItems(workspaceId: number, options: { limit: number; offset: number }) {
    ...
    const failedEmails = emailResults.filter((r) => 'status' in r && r.status === 'failed')
...
CronJobManager.scheduleJob('0 0,12 * * *', 'Low Stock Alert', async () => {
    ...
    for (const workspace of workspaces) {
        ...
        try {
            ...
            while (true) {
                ...
                console.log(
                    `Workspace ${workspace.id}: ${result.totalLowStock} low stock items found, ${result.emailStatus.totalSent - result.emailStatus.failed}/${result.emailStatus.totalSent} emails sent`
                );
